...
class RandomString
{
    ...
    public static function generate(int $length = 32, string $charset = self::CHAR_LOWER): string
    {
        ...
        if (!is_numeric($length)) {
            throw new InvalidArgumentException('random_str - Argument 1 - expected an integer');
        }
        if (!is_string($charset)) {
            throw new InvalidArgumentException('random_str - Argument 2 - expected a string');
        }
        ...
        if ($charset_max < 1) {
            ...
            throw new LogicException('random_str - Argument 2 - expected a string that contains at least 2 distinct characters');
        }
