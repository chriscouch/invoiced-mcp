(function () {
    ...
    angular.module('app.auth').controller('Setup2FAController', Setup2FAController);

    Setup2FAController.$inject = ['$scope', '$modalInstance', 'CurrentUser', 'CSRF'];

    function Setup2FAController($scope, $modalInstance, CurrentUser, CSRF) {
        $scope.countriesList = [
            {
        ...
        $scope.countryCode = '1_1';
        $scope.step = 1;

        $scope.step1 = step1;
        $scope.step2 = step2;

        function step1(phone, countryCode) {
            $scope.saving = true;
            $scope.error = false;

            // remove '_1' from country code
            countryCode = countryCode.split('_')[0];

            // retrieve a fresh CSRF token first
            CSRF(function () {
                CurrentUser.setup2FAStep1(
                    {
                        phone: phone,
        ...
        function step2(password, token) {
            $scope.saving = true;
            $scope.error = false;

            // retrieve a fresh CSRF token first
            CSRF(function () {
                CurrentUser.setup2FAStep2(
                    {
                        password: password,
                        token: token,
                    },
