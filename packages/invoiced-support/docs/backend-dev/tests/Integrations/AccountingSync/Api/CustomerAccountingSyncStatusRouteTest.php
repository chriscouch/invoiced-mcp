...
class CustomerAccountingSyncStatusRouteTest extends AppTestCase
{
    ...
    private function getRoute(): CustomerAccountingSyncStatusRoute
    {
        return new CustomerAccountingSyncStatusRoute();
    }

    public function testNoMapping(): void
    {
        $request = new Request([], [], ['model_id' => self::$customer->id()]);
        $route = $this->getRoute();
        $route->setModelClass(Customer::class);
    ...
    {
        self::$customer->metadata = (object) ['quickbooks_customer_id' => '456'];
