...
class XeroLoginStrategyTest extends AppTestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        foreach (['test', 'test65498'] as $id) {
            self::getService('test.database')->delete('Users', ['xero_claimed_id' => $id]);
        }
    ...
    private function getStrategy(): XeroLoginStrategy
    {
        $strategy = self::getService('test.xero_login_strategy');
    ...
    {
        $this->assertEquals('xero', $this->getStrategy()->getId());
    }
    ...
    public function testGetAuthorizationUrl(): void
    {
        ...
        $expected = [
            ...
            'client_id' => 'id_test',
            'state' => 'test_state',
            'redirect_uri' => 'https://invoiced.localhost:1234/auth/xero',
        ];
    ...
    public function testHandleAuthorizationCodeNewUser(): void
    {
        $client = new MockHttpClient([
            new MockResponse((string) json_encode([
                ...
                'id_token' => $this->getIdToken([
                    'sub' => 'test65498',
                    'email' => 'test+xeronew@example.com',
                    'name' => 'Bob1 Loblaw1',
                    'given_name' => 'Bob1',
                    'family_name' => 'Loblaw1',
                ]),
        ...
        $this->assertEquals('test65498', $user->xero_claimed_id);
        $this->assertEquals('test+xeronew@example.com', $user->email);
    ...
    public function testHandleAuthorizationCodeExistingUser(): void
    {
        ...
        $this->assertEquals('test', $user->xero_claimed_id);
    ...
    public function testHandleAuthorizationCodeVerifiedEmailCollision(): void
    {
        ...
        $this->assertInstanceOf(User::class, $user);
        $this->assertEquals('test2893498234', $user->xero_claimed_id);
        $this->assertEquals(self::$user->email, $user->email);
        $this->assertEquals('Bob1', $user->first_name);
        $this->assertEquals('Loblaw1', $user->last_name);
