...
class EstimateApprovedTest extends AbstractNotificationEmailTest
{
    ...
    private function getEmail(): EstimateApproved
    {
        return new EstimateApproved(self::getService('test.database'));
    }
    ...
    public function testProcess(): void
    {
        ...
        $this->assertEquals(
            [
                'subject' => 'Estimate was approved',
            ],
            $email->getMessage(self::$events)
        );
        $this->assertEquals('notifications/estimate-approved', $email->getTemplate(self::$events));
    ...
    public function testProcessBulk(): void
    {
        ...
        $this->assertEquals('notifications/estimate-approved-bulk', $email->getTemplate(self::$events));
        $this->assertEquals(
            [
                'count' => 4,
            ],
