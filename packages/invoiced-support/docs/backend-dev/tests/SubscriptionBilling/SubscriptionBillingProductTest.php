...
class SubscriptionBillingProductTest extends AppTestCase
{
    public function testInstall(): void
    {
        ...
        self::getService('test.product_installer')->install(Product::where('name', 'Subscription Billing')->one(), $company);

        // verify the settings were created
        $settings = SubscriptionBillingSettings::oneOrNull();
        $this->assertInstanceOf(SubscriptionBillingSettings::class, $settings);
        $this->assertNotNull($settings->mrr_version);
