...
final class CreditNoteViewModelFactory implements UblDocumentViewModelFactoryInterface
{
    public function make(SimpleXMLElement $xml): DocumentViewModel
    {
        $viewModel = new CreditNoteViewModel();

        // Bill From / Bill To / Ship To
        $this->parseContacts($xml, $viewModel);

        // Issue Date / Payment Terms
        $viewModel->setIssueDate(new CarbonImmutable(UblReader::xpathToString($xml, '/doc:CreditNote/cbc:IssueDate')));
        $viewModel->setPaymentTerms(UblReader::xpathToString($xml, '/doc:CreditNote/cac:PaymentTerms/cbc:Note'));
        $viewModel->setPurchaseOrder(UblReader::xpathToString($xml, '/doc:CreditNote/cac:OrderReference/cbc:ID'));
        ...
        // Notes
        $viewModel->setNotes(UblReader::xpathToString($xml, '/doc:CreditNote/cbc:Note'));

        return $viewModel;
    }

    private function parseContacts(SimpleXMLElement $xml, CreditNoteViewModel $viewModel): void
    {
        ...
        $fromAddress = $this->getPartyAddress($xml, '/doc:CreditNote/cac:AccountingSupplierParty/cac:Party');
        $formattedFromAddress = $formatter->format($fromAddress);
        $toAddress = $this->getPartyAddress($xml, '/doc:CreditNote/cac:AccountingCustomerParty/cac:Party');
    ...
    {
        return $this->getAddress(UblReader::xpathToSingle($xml, '/doc:CreditNote/cac:Delivery/cac:DeliveryLocation/cac:Address'))
            ->withOrganization((string) UblReader::xpathToString($xml, '/doc:CreditNote/cac:Delivery/cac:DeliveryParty/cac:PartyName/cbc:Name'));
    }
    ...
    private function parseLegalMonetaryTotal(SimpleXMLElement $xml, CreditNoteViewModel $viewModel): void
    {
        $currencies = new ISOCurrencies();
        $moneyParser = new DecimalMoneyParser($currencies);
        $subtotal = UblReader::xpathToString($xml, '/doc:CreditNote/cac:LegalMonetaryTotal/cbc:TaxExclusiveAmount');
        $subtotalCurrency = UblReader::xpathToString($xml, '/doc:CreditNote/cac:LegalMonetaryTotal/cbc:TaxExclusiveAmount/@currencyID');
        if ($subtotal && $subtotalCurrency) {
            $viewModel->setSubtotal($moneyParser->parse($subtotal, new Currency($subtotalCurrency)));
        }
        $total = UblReader::xpathToString($xml, '/doc:CreditNote/cac:LegalMonetaryTotal/cbc:TaxInclusiveAmount');
        $totalCurrency = UblReader::xpathToString($xml, '/doc:CreditNote/cac:LegalMonetaryTotal/cbc:TaxInclusiveAmount/@currencyID');
        if ($total && $totalCurrency) {
            $viewModel->setTotal($moneyParser->parse($total, new Currency($totalCurrency)));
        }
        $totalAllowances = UblReader::xpathToString($xml, '/doc:CreditNote/cac:LegalMonetaryTotal/cbc:AllowanceTotalAmount');
        $totalAllowancesCurrency = UblReader::xpathToString($xml, '/doc:CreditNote/cac:LegalMonetaryTotal/cbc:AllowanceTotalAmount/@currencyID');
        ...
        $totalCharges = UblReader::xpathToString($xml, '/doc:CreditNote/cac:LegalMonetaryTotal/cbc:ChargeTotalAmount');
        $totalChargesCurrency = UblReader::xpathToString($xml, '/doc:CreditNote/cac:LegalMonetaryTotal/cbc:ChargeTotalAmount/@currencyID');
        ...
        $totalTax = UblReader::xpathToString($xml, '/doc:CreditNote/cac:LegalMonetaryTotal/doc:Invoice/cac:TaxTotal/cbc:TaxAmount');
        $totalTaxCurrency = UblReader::xpathToString($xml, '/doc:CreditNote/cac:LegalMonetaryTotal/doc:Invoice/cac:TaxTotal/cbc:TaxAmount/@currencyID');
        if ($totalTax && $totalTaxCurrency) {
            $viewModel->setTotalTax($moneyParser->parse($totalTax, new Currency($totalTaxCurrency)));
        }
        $amountPaid = UblReader::xpathToString($xml, '/doc:CreditNote/cac:LegalMonetaryTotal/cbc:PrepaidAmount');
        $amountPaidCurrency = UblReader::xpathToString($xml, '/doc:CreditNote/cac:LegalMonetaryTotal/cbc:PrepaidAmount/@currencyID');
        if ($amountPaid && $amountPaidCurrency) {
            $viewModel->setAmountPaid($moneyParser->parse($amountPaid, new Currency($amountPaidCurrency)));
        }
        $balance = UblReader::xpathToString($xml, '/doc:CreditNote/cac:LegalMonetaryTotal/cbc:PayableAmount');
        $balanceCurrency = UblReader::xpathToString($xml, '/doc:CreditNote/cac:LegalMonetaryTotal/cbc:PayableAmount/@currencyID');
        if ($balance && $balanceCurrency) {
            $viewModel->setBalance($moneyParser->parse($balance, new Currency($balanceCurrency)));
        }
    }

    private function parseLineItems(SimpleXMLElement $xml, CreditNoteViewModel $viewModel): void
    {
        $currencies = new ISOCurrencies();
        $moneyParser = new DecimalMoneyParser($currencies);
        foreach (UblReader::xpath($xml, '/doc:CreditNote/cac:CreditNoteLine') as $lineItem) {
            $viewModelLine = [
                ...
                'quantity' => UblReader::xpathToString($lineItem, 'cbc:CreditedQuantity'),
                'unit_cost' => null,
                'total' => null,
            ];
