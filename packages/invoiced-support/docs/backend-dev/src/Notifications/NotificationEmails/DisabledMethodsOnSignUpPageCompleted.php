...
class DisabledMethodsOnSignUpPageCompleted extends AbstractNotificationEmail
{
    const THRESHOLD = 3;

    protected function getSubject(): string
    {
        return 'Sign up page completed but some payment methods were disabled';
    }

    public function getTemplate(array $events): string
    {
        if (count($events) > static::THRESHOLD) {
            return 'notifications/sign-up-page-completed-payment-methods-disabled-bulk';
        }

        return 'notifications/sign-up-page-completed-payment-methods-disabled';
    }
