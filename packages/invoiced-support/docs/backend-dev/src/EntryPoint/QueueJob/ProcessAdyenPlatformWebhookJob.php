...
class ProcessAdyenPlatformWebhookJob extends AbstractResqueJob implements MaxConcurrencyInterface, TenantAwareQueueJobInterface
{
    ...
    {
        // use company time zone if we're in a tenant context (reports are not)
        if ($this->tenant->has()) {
        ...
        $this->dispatcher->dispatch(new AdyenPlatformWebhookEvent($data));
    }
    ...
    {
        return 'adyen_platform_webhook:'.$args['tenant_id'];
    }
