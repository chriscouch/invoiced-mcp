...
class XeroInvoiceExtractor extends AbstractXeroExtractor
{
    ...
    public function getObjects(AccountingSyncProfile $syncProfile, ReadQuery $query): Generator
    {
        ...
        try {
            ...
            do {
                $params['page'] = $page;
                $objects = $this->xeroApi->getMany('Invoices', $params, $headers);
    ...
    {
        $pdf = $this->readPdfs ? $this->getPdf($transaction->InvoiceID) : null;

        return new AccountingJsonRecord($transaction, (object) ['pdf' => $pdf]);
    }

    /**
     * Retrieves the invoice PDF from Xero.
     */
    private function getPdf(string $id): string
    {
        try {
            return $this->xeroApi->getPdf('Invoices', $id);
        } catch (IntegrationApiException $e) {
    ...
    {
        return $accountingObject->document->InvoiceID;
    }
}
