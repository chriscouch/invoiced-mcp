...
class BusinessCentralCreditMemoExtractor extends AbstractBusinessCentralExtractor
{
    ...
    public function getObjects(AccountingSyncProfile $syncProfile, ReadQuery $query): Generator
    {
        try {
            ...
            if ($query->startDate) {
                $filter[] = 'creditMemoDate ge '.$query->startDate->toDateString();
            }
            if ($query->endDate) {
                $filter[] = 'creditMemoDate le '.$query->endDate->toDateString();
            }
            if ($query->openItemsOnly) {
                $filter[] = "status ne 'Canceled' and status ne 'Paid' and status ne 'Corrective'";
            }

            // NOTE: open item filtering is not available due to there being no balance field
            $filter = implode(' and ', $filter);

            $objects = $this->businessCentralApi->getSalesCreditMemos($this->account, array_filter([
                '$filter' => $filter,
                '$expand' => 'salesCreditMemoLines',
            ]));

            // TODO: pagination
            AccountingSyncStatus::setMessage($syncProfile->getIntegrationType(), 'Syncing Credit Memos from Business Central ('.number_format(count($objects)).' records)');
    ...
    {
        // If the credit note already exists and has a PDF then we are not going to reload the PDF.
        ...
        try {
            return $this->businessCentralApi->getPdf($this->account, 'salesCreditMemos', $id);
