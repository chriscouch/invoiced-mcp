 ...
 * to be able to work with invoices inside
...
define(['N/log', 'tmp/src/Models/ConnectorFactory.js'], function (
    ...
    return {
        patch(key: null | string, data: HasNumberRow): null | InvoicedTransaction {
            const connection = ConnectorFactory.getInstance();
            const response = connection.send('POST', 'invoices/accounting_sync', key, data);
            log.debug('Response', response);
            return response as null | InvoicedTransaction;
        },
    };
});
